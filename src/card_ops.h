...
inline unsigned long evaluate_hand(const unsigned long& cards) {
    ...
    case 5:
    {
        ...
        if(straight_flush_mask) [[unlikely]] return STRAIGHT_FLUSH_RESULT | (1UL << (first_bit_idx(straight_flush_mask) % 16));
        ...
        if(straight_mask) [[unlikely]] return STRAIGHT_RESULT | first_bit_idx(straight_mask) % 16;
    }
    [[likely]] case 4:
    case 3:
    case 2: 
    {
        ...
        if(and_ranks) return QUADS_RESULT | and_ranks << 16 | first_bit_idx(or_ranks ^ and_ranks);
...
inline unsigned long evaluate_hand_no_branch(const unsigned long& cards) {
    ...
    auto straight_flush_mask = detect_straight(cards);    
    if(straight_flush_mask) [[unlikely]] return STRAIGHT_FLUSH_RESULT | first_bit_idx(straight_flush_mask) % 16;
